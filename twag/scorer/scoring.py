...
from twag.taxonomy import SignalTier
...
def triage_tweet(
    ...
    categories = data.get("categories") or [data.get("category", SignalTier.NOISE)]
...
def triage_tweets_batch(
    ...
    for item in data:
        # Handle both old "category" (string) and new "categories" (array) format
        categories = item.get("categories") or [item.get("category", SignalTier.NOISE)]
        if isinstance(categories, str):
            categories = [categories]
