...
def _merge_document_media(media_items: list[dict[str, Any]]) -> bool:
    ...
    try:
        ...
        log.warning("Document text summarization failed", exc_info=True)
    ...
    def _submit_enrichment(tweet_id: str, tweet_row: sqlite3.Row) -> None:
        ...
        if text_pool:
            ...
            try:
                ...
                log.exception("Inline enrichment failed for tweet %s", tweet_id)
            _complete_task(tweet_id)
    ...
    def _submit_article(tweet_id: str, tweet_row: sqlite3.Row) -> None:
        ...
        if text_pool:
            ...
            try:
                ...
                log.exception("Inline article processing failed for tweet %s", tweet_id)
            _complete_task(tweet_id)
            ...
            if allow_summarize and len(content) > 500 and not is_tier1 and result.score >= 5:
                if text_pool:
                    ...
                    try:
                        ...
                        log.warning("Inline tweet summarization failed for %s", result.tweet_id, exc_info=True)
        ...
        for future in as_completed(list(all_futures.keys())):
            ...
            if tag == "summary":
                ...
                try:
                    ...
                    log.exception("Async summary failed for tweet %s", tweet_id)
                _complete_task(tweet_id)
            elif tag == "media":
                tweet_id = data
                try:
                    ...
                    log.exception("Async media analysis failed for tweet %s", tweet_id)
                _complete_task(tweet_id)
            elif tag == "article":
                tweet_id, row = data
                try:
                    ...
                    log.exception("Async article processing failed for tweet %s", tweet_id)
                _complete_task(tweet_id)
            elif tag == "enrich":
                tweet_id, row = data
                try:
                    ...
                    log.exception("Async enrichment failed for tweet %s", tweet_id)
                _complete_task(tweet_id)
    finally:
        if triage_pool:
